...
| File | Description |
|------|-------------|
| `Module2b_Multimodal_PDF_Extraction.ipynb` | the teaching notebook (executed, with outputs) |
...
| `data/pairs_gold.csv` | the 92-row RAPIDS comparison subset, filtered to these 8 papers |
...
6. Scoring by selectivity ordering recovery (the fair metric when the gold curates its pairs unevenly; exact pair matching is not), then lifting the multimodal arm to 0.86 by reading thoroughly and unioning two reads (a plain and an exhaustive pass, the simplest form of a multi-agent read).
...
For live mode (set `USE_CACHE = False`), paste your OpenRouter key into the `API_KEY = ""` line near the top of the notebook, for example `"sk-or-v1-..."`. That is the only step. You can instead set an `OPENROUTER_API_KEY` environment variable and leave the line blank.
