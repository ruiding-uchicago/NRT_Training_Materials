...
The broadened query matches ~6,700 open-access battery papers; the shipped cache holds full text for 337 and an extracted table of 295. On a single 80/20 split (seed 7, same style as Modules 1a–1d): predicting log10 specific capacity gives **R² ≈ 0.74**, and classifying high-capacity (>500 mAh/g) vs conventional gives **accuracy ≈ 0.90** (baseline 0.64). The top drivers are the chemistry family (Li-S / sulfur high), electrode role, voltage window, and cycle number. Energy density and other near-answer quantities are deliberately excluded.
